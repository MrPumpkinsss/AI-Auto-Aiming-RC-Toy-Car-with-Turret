...
class ServoControl:
    ...
    def __init__(self, x_axis_pin, y_axis_pin, x_range=(0, 270), y_range=(0, 270)):
        ...
        # Initialize offset for X and Y axes
        self.x_offset = 6  # Offset for the x-axis
        self.y_offset = 10  # Offset for the y-axis
        
    def _angle_to_pulsewidth(self, angle):
        """
        Converts an angle to a pulse width in microseconds.

        :param angle: Target angle (0 to 270)
        :return: Pulse width value (500 to 2500 microseconds)
        """
        # Scale 0-270 to pulse width range of 500-2500
    ...
    def move_servo(self, axis, angle):
        ...
        angle = round(angle, 2)
        if axis == 'x':
            # Apply offset and clamp to range
            angle = max(self.x_min, min(angle + self.x_offset, self.x_max))
            pin = self.x_axis_pin
        elif axis == 'y':
            # Apply offset and clamp to range
            angle = max(self.y_min, min(angle + self.y_offset, self.y_max))
            pin = self.y_axis_pin
        else:
