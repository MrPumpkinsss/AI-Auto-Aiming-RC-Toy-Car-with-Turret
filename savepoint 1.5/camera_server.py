...
class CameraServer:
    def __init__(self, flask_port=5000, socket_port=10000):
        ...
        self.last_time = time.time()  # Timestamp of the last PID update
        self.error_sum_x = 0  # Integral term for x-axis
        self.error_sum_y = 0  # Integral term for y-axis
        self.last_error_x = 0  # Last error term for x-axis
        self.last_error_y = 0  # Last error term for y-axis

        self.kp_x = 0.01  # Proportional gain for x-axis
        self.ki_x = 0  # Integral gain for x-axis
        self.kd_x = 0  # Derivative gain for x-axis
    ...
    def aim_at_target(self, target_x, target_y, center_x, center_y):
        ...
        # Calculate the error between the target and the frame center
        ...
        self.error_sum_x += error_x * delta_time  # Integral term
        delta_error_x = (error_x - self.last_error_x) / delta_time  # Derivative term
        pid_output_x = (
            self.kp_x * error_x +
        ...
        self.error_sum_y += error_y * delta_time  # Integral term
        delta_error_y = (error_y - self.last_error_y) / delta_time  # Derivative term
        pid_output_y = -(
            self.kp_y * error_y +
        ...
        self.gimbal_x = max(0, min(270, self.gimbal_x - pid_output_x))  # Clamp to valid range
        self.gimbal_y = max(0, min(270, self.gimbal_y - pid_output_y))  # Clamp to valid range
            ...
            try:
                ...
                for contour in contours:
                    if len(contour) >= 5:
                        ...
                        if area > 100:  # Only process contours with sufficient area
                            try:
                                ellipse = cv2.fitEllipse(contour)
                                if area > largest_area:  # Store the largest ellipse
                                    largest_area = area
                ...
                # Update the target position with the center of the largest ellipse
                if largest_ellipse is not None:
                    (x, y), _, _ = largest_ellipse  # Center of the ellipse
                    self.target_position = (int(x), int(y))  # Update target position
                    cv2.ellipse(frame, largest_ellipse, (255, 0, 0), 2)  # Draw the largest ellipse
                    cv2.circle(frame, (int(x), int(y)), 5, (0, 0, 255), -1)  # Mark the center
                    cv2.putText(frame, f"Target: ({int(x)}, {int(y)})", (int(x) + 10, int(y) - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                else:
                # No valid target found, set target position to None
                    self.target_position = None
